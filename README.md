...
<!-- ![Node Version](https://img.shields.io/badge/Node.js-LTS-green.svg) -->
...
4.  [Configuration](#4-configuration-🛠️)
...
Locate the "Ask the Default AI Agent" section.
Type your question into the "Your Question:" input field.
Click the "Ask" button or press the Enter key.
The UI will display "Thinking..." while waiting for the backend.
The agent's response will appear in the "Answer:" paragraph below the button.
Error messages from the backend will also be displayed here.
Create New Agent Configuration:
Locate the "Create New Agent" section.
Agent Name: Enter a name for your new agent. Only alphanumeric characters, underscores (_), and hyphens (-) are allowed.
LLM API: Select the target API (e.g., gemini).
Model Name: Enter the specific model identifier (e.g., gemini-1.5-flash-latest).
Click the "Create Agent" button.
A status message below the button will indicate success (Agent '...' created...) or failure (Error: ...).
Result: If successful, this creates a new subdirectory within ./agentic/ on the server's filesystem containing agent.py, .env, and __init__.py templates for the specified agent.
